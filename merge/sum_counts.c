...
int process_line(int first, char* line, char* delim, long** vals, config_settings config_def, int print_coord)
{
	...
	while(tok != NULL)
	{
		...
		if(i >= config_def.base_col_start)
		{
			if(strcmp(config_def.op, "sum") == 0 || strcmp(config_def.op, "mean") == 0)
				(*vals)[(*col_idx)-config_def.base_col_start] += atol(tok);
		}
		i++;
		tok = strtok(NULL,delim);
	}
...
config_settings setup(int argc, char*** argv)
{
	...
	config_def.base_col_start=6;
	config_def.axis=ROW_AXIS;
	config_def.op = "sum";
	config_def.label = "";
	...
	while(opt_ != -1)
	{
		...
		case 'a':
            ...
            //fprintf(stderr,"setting axis to %d\n",config_def.axis);
			break;
		case 'o':
			config_def.op = optarg;
            //fprintf(stderr,"setting operation to %s\n",config_def.op);
			break;
		case 'l':
    ...
    fprintf(stderr,"axis:%d,operation:%s,label:%s,base_col_start:%d\n",config_def.axis,config_def.op,config_def.label,config_def.base_col_start);
    fflush(stderr);
...
int main(int argc, char** argv)
{
	...
	while(bytes_read != -1)
	{
		...
		if(first)
		{
			...
			vals = calloc(num_vals,sizeof(long));
			first = 0;
		...
		if(config_def.axis == COL_AXIS)
		{
			if(strcmp(config_def.op, "mean") == 0)
				fprintf(stdout,"\t%.3f",((double)vals[0])/num_vals);
			else
				fprintf(stdout,"\t%lu",vals[0]);
			printf("\n");
			fflush(stdout);
		}
	...
	if(config_def.axis == ROW_AXIS)
	{
		...
		{
			if(strcmp(config_def.op, "mean") == 0)
				fprintf(stdout,"\t%.3f",((double)vals[j])/num_rows);
			else
				fprintf(stdout,"\t%lu",vals[j]);
		}
		printf("\n");
		fflush(stdout);
	}
