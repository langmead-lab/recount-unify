...
max_tranche_num=$2
#e.g. G029.G026.R109.F006.20190220.gtf.exonIDs2annotations.tsv
...
for i in {0..${max_tranche_num}}; do
    ...
    pushd $tdir
    pypy create_exon_sums_by_study_splits.py ids.tsv all.exon_counts.rejoined.tsv.gz all.exon_counts.rejoined.tsv.gz.accession_header ${outdir}/tranche${i} > exon_sums.splits.${i}.jobs
...
cat ${rejoined_exon_coords} | ${dirname}/../rejoin/python3 map_exon_sum_rows_to_annotations.py $exonids2annotations $annotations > ${bitmasks_file}.tsv

bitmask_coords_file=${bitmasks_file}.coords
#make 1-based, not BED
cut -f 2-4 $bitmasks_file | perl -ne 'chomp; ($c,$s,$e)=split(/\t/,$_); $s++; print "$c\t$s\t$e\n";' > ${bitmask_coords_file}.tsv

#create per-annotation split jobs;
echo -n "" > exon_sums.per_annotation.splits.jobs
for sfile in `ls ${outdir}/tranche?/*.*.gz`; do
    study=`echo "$sfile" | cut -d'.' -f 2`
...
#/bin/bash -x split_out_exons.sh G026,G029,R109,ERCC,SIRV all.exon_counts.rejoined.tsv.gz.coords.bitmasks2 1709834 ERP001942 examples_done/ERP001942.gz examples_done/ all.exon_counts.rejoined.tsv.gz.coords.bitmasks2_1base_coords > ERP001942.final_annotation_split.run 2>&1
    echo "/usr/bin/time -v /bin/bash -x ${dir}/../rejoin/split_out_exons.sh \"${annotations}\" exon_counts.coords.bitmasks $num_exon_rows $study $sfile $final_outdir ${bitmask_coords_file}.tsv > ${study}.annot_split.run 2>&1" >> exon_sums.per_annotation.splits.jobs
done 
