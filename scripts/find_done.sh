...
find -L $search_dir -name "*.done" | fgrep -v "fastq_removal" | sort | perl -ne 'BEGIN { $dest_dir="'${destination_dir}'"; } chomp; $fp=$_; $fp=~s/\.done$//; @f=split(/\//,$fp); $f=pop(@f); $fp2=join("/",@f); $acc=pop(@f); $acc_loworder=pop(@f); $study=pop(@f); $loworder=pop(@f); ($proj,$input_id,$attempt)=split(/_/,$f); $f_wo_attempt_num=$f; $f_wo_attempt_num=~s/(\d+)$//; $attempt_num=$1; print "$study\t$f\t$loworder\t$f_wo_attempt_num\t$attempt_num\t$acc_loworder\n"; if(!$h{$study.$f_wo_attempt_num} || $h{$study.$f_wo_attempt_num}->[0] > $attempt_num) { $h{$study.$f_wo_attempt_num}=[$attempt_num,$fp2,$loworder,$study,$acc_loworder,$acc,$fp]; } END { for $v (values %h) { ($attempt_num,$fp2,$loworder,$study,$acc_loworder,$acc,$fp)=@$v; `mkdir -p $dest_dir/$loworder/$study/$acc_loworder`; print "$fp\n"; `ln -fs $fp2 $dest_dir/$loworder/$study/$acc_loworder/`; }}' > ${destination_dir}/attempts.moved.${date}
